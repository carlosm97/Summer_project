...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug 20 16:48:11 2021
...
SM=reading_summary('../stars/HD34078/4661/00_HD34078_4661.txt')
...
f_HJD,f_fm=np.vstack(f_HJD),np.vstack(f_fm)
for n in range(1,50):
...
maxi,mini=max(STD_f_1),min(STD_f_1)
...
sa[1].bar((bins_f_10[1:]+bins_f_10[:-1])/2,H_f_10/max(H_f_10),alpha=0.3,width=np.mean(bins_f_10[1:]-bins_f_10[:-1]),label='FIES')
#sa[1].bar((bins_m_10[1:]+bins_m_10[:-1])/2,H_m_10/max(H_m_10),alpha=0.3,width=np.mean(bins_m_10[1:]-bins_m_10[:-1]),label='HERMES')
sa[1].legend()
...
sa[2].bar((bins_f_25[1:]+bins_f_25[:-1])/2,H_f_25/max(H_f_25),alpha=0.3,width=np.mean(bins_f_25[1:]-bins_f_25[:-1]),label='FIES')
#sa[2].bar((bins_m_25[1:]+bins_m_25[:-1])/2,H_m_25/max(H_m_25),alpha=0.3,width=np.mean(bins_m_25[1:]-bins_m_25[:-1]),label='HERMES')
sa[2].legend()



sa[3].set_title('Random selection')
...
plt.savefig('../stars/HD34078/steps_4661.png')
# In[prueba]

H, bins = np.histogram(STD_f_1,bins=20)
plt.figure()
...
plt.savefig('../stars/HD34078/Total_study_4661.png')
